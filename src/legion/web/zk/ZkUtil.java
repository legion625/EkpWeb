...
public class ZkUtil {
	...
	/**
	 * // XXX 待更新新legionmodule
	 * @param _cbb
	 * @param _idEnums
	 * @param _containsBlank
	 */
	public static void initCbb(Combobox _cbb, IdEnum[] _idEnums, boolean _containsBlank) {
		if (_cbb == null)
			return;
		_cbb.getChildren().clear();
		...
		for (IdEnum _idEnum : _idEnums) {
			if (DataFO.isEmptyString(_idEnum.getId()) || "UNDEFINED".equalsIgnoreCase(_idEnum.getId()))
				continue;
			Comboitem cbi = new Comboitem(_idEnum.getName());
			cbi.setValue(_idEnum);
			_cbb.appendChild(cbi);
		}
	}
